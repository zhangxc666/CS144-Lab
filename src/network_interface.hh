...
private:
    ...
    Address ip_address_;
    std::map<uint32_t , std::pair<EthernetAddress, uint64_t>> IptoMac{}; // 映射ip->(mac地址,time)
    std::map<uint32_t , std::queue<InternetDatagram>> IP_queue{};   // 存储IP数据报
    std::map<uint32_t , uint64_t> lastARPRequestTime{};     // 上一个ARP请求时间
    std::queue<EthernetFrame> frame_queue{};   // 存储以太帧
    uint64_t curTime{0};
public:
