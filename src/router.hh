...
class AsyncNetworkInterface : public NetworkInterface { // receive和send都有队列可以存储，是异步的
...
public:
    ...
    void recv_frame(const EthernetFrame &frame) {  // 将收到的frame转换成IP数据报后，放入队列中
    ...
    std::optional<InternetDatagram> maybe_receive() { // 从队列中取出IP数据报，进行处理
